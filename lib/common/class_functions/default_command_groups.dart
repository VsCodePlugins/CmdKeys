...
Future <List<ModelCommandGroup>> createDefaultCommandGroups({required String keyBaseCommandGroup, required SharedPreferences sharedPreferences}) async {
 List<ModelCommandGroup> commandsGroup = [];
  ...
  for (var cmdGroupModel in commandsGroup) {
     cmdGroupModel.saveAs(keyBaseCommandGroup:keyBaseCommandGroup , sharedPreferences: sharedPreferences);
  List<ModelCommand> listModelCmd = cmdGroupModel.onlyType.getDefaultCommands;
  for (ModelCommand  cmdModel in listModelCmd) {
    await cmdModel.saveAs(commandGroupID: cmdGroupModel.id,sharedPreferences: sharedPreferences);
